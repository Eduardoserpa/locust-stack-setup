...
class MatrixRoomCreatorUser(MatrixUser):
    ...
    def create_rooms_for_user(self):
        ...
        # Load username and password from CSV
        self.set_user(user["username"])  # self.matrix_client.user = "user.000000"
        self.matrix_client.password = user["password"]

        # Synapse requires full user ID format for login in nio
        # But set_user will handle the domain based on what's returned
        # For now, just use the local username and let the server respond with full format
        self.matrix_client.matrix_domain = "matrix.local"

        logging.info("User loaded from CSV - username: [%s]", self.matrix_client.user)

        if self.matrix_client.user is None or self.matrix_client.password is None:
            logging.error("CSV loading failed - username/password empty")
            return
        ...
        if isinstance(response, LoginError):
            logging.error("Login FAILED - User: [%s], Status: %s, Message: %s",
                        self.matrix_client.user, response.status_code, response.message)
            return

        logging.info("Login SUCCESS - User: [%s], User ID: [%s], Token acquired",
                    self.matrix_client.user, self.matrix_client.user_id)
